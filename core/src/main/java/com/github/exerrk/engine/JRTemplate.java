 ...
 * {@link com.github.exerrk.engine.JRTemplate} instances. A reference to a
 * style template in a report is represented by a
 * {@link com.github.exerrk.engine.JRReportTemplate} instance. Such references can
 * be added to a {@link com.github.exerrk.engine.design.JasperDesign} object
 * via the {@link com.github.exerrk.engine.design.JasperDesign#addTemplate(JRReportTemplate) addTemplate(JRReportTemplate)} method, and the list of
 * template references can be obtained by calling the {@link com.github.exerrk.engine.design.JasperDesign#getTemplates() getTemplates()} method.
 * <p/>
 * In more complex cases, style templates can be injected into a report by using the built-in
 * {@link com.github.exerrk.engine.JRParameter#REPORT_TEMPLATES REPORT_TEMPLATES} report parameter.
 * This parameter expects as value a collection (as
 * in <code>java.util.Collection</code>) of {@link com.github.exerrk.engine.JRTemplate}
 * instances. The user report fill code can either load style templates on its own from
 * template XML files using
 * {@link com.github.exerrk.engine.xml.JRXmlTemplateLoader}, or instantiate style
 * template objects by any other means, and pass the dynamically loaded style template list
 * to the report fill process using the {@link com.github.exerrk.engine.JRParameter#REPORT_TEMPLATES REPORT_TEMPLATES} parameter.
 * <p/>
 * Style templates passed to a report via the {@link com.github.exerrk.engine.JRParameter#REPORT_TEMPLATES REPORT_TEMPLATES}
